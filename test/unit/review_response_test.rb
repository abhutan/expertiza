...
class ReviewResponseTest < ActiveSupport::TestCase
  ...
  test "method_questionnaire" do
    @questionnaire = questionnaires(:questionnaire0)
    @assignment = assignments(:assignment0)
    reviewrespmap = ReviewResponse.new
  ...
  test "method_get_export_fields" do
    ...
    fields_2 = ReviewResponse.get_export_fields(1)
    assert_equal fields_2[0], fields_1[0]
    assert_equal fields_2[1], fields_1[1]
  end

  test "method_get_import_fields" do
    p = ReviewResponse.import(['student2','student2'], 2, "827400667")
    assert_equal p , nil
  end

  test "method_get_import_raise_less_than_two_items" do
    assert_raise (ArgumentError) {ReviewResponse.import([''], 2, "827400667")}
  end

  test "method_get_import_with_incorrect_assignment_id" do
    @assignment = '123'
    assert_raise (ActiveRecord::RecordNotFound) {ReviewResponse.import(['student2','student2'], 2, @assignment)}
  end

  test "method_get_import_with_incorrect_user" do
    @user1 = 'student30'
    @user = 'student20'
    assert_raise (ImportError) {ReviewResponse.import([@user1,@user,'student2'], 2, "827400667")}
  end

  test "method_get_import_with_incorrect_reviewer" do
    @user = 'abc'
    assert_raise (ImportError) {ReviewResponse.import(['student2',@user], 2, "827400667")}
  end

  test "method_get_import_team_assignment_with_no_reviewee" do
    @assignment = assignments(:assignment2)
    assert_raise (ImportError) {ReviewResponse.import(['student1','student2'], 2, @assignment)}
  end

  test "method_show_feedback" do
    @questionnaire = questionnaires(:questionnaire0)
    @assignment = assignments(:assignment0)
    rev = ReviewResponse.new
  ...
  test "method_add_reviewer" do
        p = ReviewResponse.add_reviewer(299716733,148111809,108022375)
        assert p.save
  end
